...
io.on('connection', function(socket) {
  ...
  socket.on('check-token', function(externalToken) {
    ...
    var gateConn = new ClientInterface(externalToken, function() {

      console.info('Checking user...', gateConn);
      io.emit('check-token', {message: 'Checking user...'});

      gateConn.user_exists(function(exists) {
    ...
    connections[socket.id] = gateConn;
  });

  socket.on('run-auth', function(msg) {
    console.log('run-auth: ', msg);

    var gateConn = connections[socket.id];

    console.log('run-auth: ', gateConn);

    io.emit('run-auth', msg);
  });

  socket.on('check-status', function(msg) {
    console.log('check-status: ', msg);
    io.emit('check-status', msg);
  });
